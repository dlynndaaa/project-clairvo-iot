...
// Pastikan file .env.local Anda sudah benar isinya
...
export default function Dashboard({ user, onLogout }: { user: any; onLogout: () => void }) {
  // --- 1. INISIALISASI DATABASE YANG AMAN (PERBAIKAN UTAMA) ---
  // Menggunakan useMemo agar tidak error "App already exists" saat refresh
  const db = useMemo(() => {
    try {
  ...
  useEffect(() => {
    // Jika db belum siap, jangan jalankan listener
    if (!db) return;
    ...
    const unsubscribeSensor = onValue(sensorRef, (snapshot) => {
      ...
      if (data) {
        ...
        const formattedData = {
          ...
          // Pastikan fan_status dibaca sebagai boolean (true/false)
          fan_status: data.fan_status === true || data.fan_status === 'true',
      ...
      console.error('Error loading sensor data. Cek Rules Firebase Anda:', error);
      setIsLoading(false);
    });
    ...
    const unsubscribeSettings = onValue(settingsRef, (snapshot) => {
      const data = snapshot.val();
      if (data && typeof data.is_auto_mode !== 'undefined') {
         // Handle boolean atau string 'true'/'false'
         const autoMode = data.is_auto_mode === true || data.is_auto_mode === 'true';
  ...
  // 4. LOGIKA FIREBASE: Update Mode Otomatis
  ...
  // 4B. LOGIKA FIREBASE: Control Kipas Manual
  ...
  // 5. Logic Status Kualitas Udara (Tampilan)
  ...
  // --- JSX RENDER (TIDAK ADA YANG DIUBAH DARI PERMINTAAN ANDA) ---
  return (
